...
void computeForceFromNode(const OctreeNode* node, Body& b, double theta, ForceField* forcefield) {
    ...
    if (forcefield->type != GRAVITY) return;
    ...
    // Calculate distance from node to body b, use squared distance to avoid sqrt()
    constexpr double softening = 1e-5; // Avoid division by 0 
    const double dist2 = norm2(r) + softening * softening;
    ...
    */
    if (node->isLeaf()) {
        ...
        // Compute force and add this contribution to b.force
        b.force += forcefield->gravity(b.mass, m, r);
        return;
    }
    ...
    */
    const double s = 2.0 * node->bounds.halfSize;
